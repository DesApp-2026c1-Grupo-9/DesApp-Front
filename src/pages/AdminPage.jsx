...
import {
  PersonAdd, School, MenuBook, Edit, Delete, Add, AdminPanelSettings, Refresh, RemoveCircleOutline,
...
function CarrerasTab() {
  ...
  const openPlanDialog = async (carrera) => {
    ...
    } catch { setPlanes([]); setAllMaterias([]); }
  };
      ...
      <Dialog open={planDialog.open} onClose={() => setPlanDialog({ ...planDialog, open: false })} maxWidth="md" fullWidth>
        ...
        <DialogContent>
              ...
              <TableBody>
                {planes.map((p) => (
                  <TableRow key={p.id}>
                    ...
                    <TableCell align="center">
                      <IconButton size="small" onClick={() => openMateriasEdit(p)} title="Gestionar materias">
                        <MenuBook fontSize="small" />
                      </IconButton>
                      <IconButton size="small" onClick={() => handleDeletePlan(p.id)} color="error" title="Eliminar plan">
                        <Delete fontSize="small" />
                      </IconButton>
                    </TableCell>
                  </TableRow>
                ...
                {planes.length === 0 && (
                  <TableRow>
                    <TableCell colSpan={4} align="center" sx={{ py: 4 }}>No hay planes de estudio</TableCell>
                  </TableRow>
                )}
              </TableBody>
            </Table>
          ...
          <Dialog open={materiasEditDialog.open} onClose={() => setMateriasEditDialog({ ...materiasEditDialog, open: false })} maxWidth="md" fullWidth>
              ...
              <Paper variant="outlined" sx={{ p: 2, mb: 2, bgcolor: 'grey.50' }}>
                ...
                <Box sx={{ display: 'flex', gap: 2 }}>
                  <Autocomplete
                    ...
                    size="small"
                    options={allMaterias
                      .filter((m) => !materiasEditDialog.plan?.materias?.some((pm) => pm.id === m.id))
                    ...
                    noOptionsText={
                      allMaterias.filter((m) => !materiasEditDialog.plan?.materias?.some((pm) => pm.id === m.id)).length === 0
                        ? 'Todas las materias ya están asignadas'
                        : 'Sin resultados'
                    }
                    renderInput={(params) => (
                      <TextField {...params} label="Buscar materia" placeholder="Escribí el nombre..." />
                    )}
                  />
                  <Button variant="contained" startIcon={<Add />} onClick={() => handleAddMateriaToPlan(materiasEditDialog.plan?.id)} disabled={!materiaToAdd} sx={{ minWidth: 130 }}>
                    Asignar
                  </Button>
                </Box>
              </Paper>
                ...
                <Table size="small">
                  <TableHead>
                    <TableRow>
                      ...
                      <TableCell width={80}>Año</TableCell>
                    ...
                    {!materiasEditDialog.plan?.materias?.length ? (
                      <TableRow>
                        <TableCell colSpan={4} align="center" sx={{ py: 4 }}>
                          <Typography color="text.secondary">Este plan no tiene materias asignadas</Typography>
                        </TableCell>
                      </TableRow>
                    ) : (
                      [...(materiasEditDialog.plan?.materias || [])]
                        .sort((a, b) => a.anio - b.anio || a.nombre.localeCompare(b.nombre))
                        .map((m) => (
                        <TableRow key={m.id}>
                          ...
                          <TableCell>
                            <Chip label={m.tipo} size="small" color={m.tipo === 'anual' ? 'info' : 'secondary'} sx={{ textTransform: 'capitalize' }} />
                          </TableCell>
                          <TableCell align="center">
                            <IconButton size="small" color="error" onClick={() => handleRemoveMateriaFromPlan(materiasEditDialog.plan?.id, m.id)} title="Remover del plan">
            ...
            <DialogActions>
              <Button onClick={() => setMateriasEditDialog({ ...materiasEditDialog, open: false })}>Cerrar</Button>
            </DialogActions>
          </Dialog>
        </DialogContent>
        <DialogActions>
