...
const MaterialCard = ({ material, currentUserId, isActive = true, onRate, onEdit, onDelete, onDenunciar, onViewDetail }) => {
  ...
  const contentElement = (
    <Box sx={{ flex: 1, minWidth: 0 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 0.5, flexWrap: 'wrap', justifyContent: { xs: 'center', sm: 'flex-start' } }}>
        <Typography variant="h6" sx={{ fontSize: '1rem', fontWeight: 600, wordBreak: 'break-word' }}>
          {material.titulo}
        </Typography>
        {isOwner && (
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: { xs: 'center', sm: 'flex-start' }, gap: 1, flexWrap: 'wrap', minWidth: 0 }}>
        <Chip
  ...
  return (
    ...
    >
      ...
      {/* Mobile: icon+buttons row | content | likes */}
      ...
      <Box sx={{ display: { xs: 'flex', sm: 'none' }, alignItems: 'center', justifyContent: 'center', gap: 1, mt: 1 }}>
        <IconButton
          size="small"
          onClick={() => handleRate(1)}
          color={material.userRating === 1 ? 'primary' : 'default'}
